...
function fill_in_check_table(check_table, slot, player_job)
	for _, gear in pairs(slot) do
		for _, sets in pairs(gear.set_list) do
			for job, set_list in pairs(sets) do
				if job == player_job then
					for _, set in pairs(set_list) do
						local idle_gear = set == "IdleRegen"
						local mode_gear = set:startswith("Mode")
						if idle_gear or mode_gear then
							if check_table[gear.name] == nil then 
								check_table[gear.name] = {} 
								check_table[gear.name].idle_gear = false
								check_table[gear.name].mode_sets = {}
							end
							if idle_gear then
								check_table[gear.name].idle_gear = true
								check_table[gear.name].idle_bag = gear.bag
							elseif mode_gear then
								table.insert(check_table[gear.name].mode_sets, { set_name = set, bag = gear.bag } )
							end
						end
					end
				end
...
function check_for_gear_in_multiple_slots(check_table, slot, player_job, check_table_name, slot_name)
	for gear_name, gear in pairs(check_table) do
		if gear.idle_gear then
			for _, gear2 in pairs(slot) do
				if gear2.name == gear_name then
					if gear2.bag == gear.idle_bag then
						for _, sets in pairs(gear2.set_list) do
							for job, set_list in pairs(sets) do
								if job == player_job then
									for _, set in pairs(set_list) do
										if set:startswith("Mode") then
											windower.add_to_chat(122, gear_name .. " [" .. check_table_name .. "][IdleRegen] & [" .. slot_name .. "][" .. set .. "]")
										end
									end
								end
							end
		...
		if gear.mode_sets ~= nil then
			for _, mode_set in pairs(gear.mode_sets) do
				for _, gear2 in pairs(slot) do
					if gear2.name == gear_name then
						if gear2.bag == mode_set.bag then
							for _, sets in pairs(gear2.set_list) do
								for job, set_list in pairs(sets) do
									if job == player_job then
										for _, set in pairs(set_list) do
											if not set:startswith("Mode") and set ~= "IdleRegen" then
												windower.add_to_chat(122, gear_name .. " [" .. check_table_name .. "][" .. mode_set.set_name .. "] & [" .. slot_name .. "][" .. set .. "]")
											end
										end
									end
								end
...
function check_multi_slots(player_job)
	windower.add_to_chat(122, "Checking for gear in multiple slots")
	-- [name] = { idle_gear = true, idle_bag = "", mode_sets = { set = "", bag = "" } }
	local ear_1_check_table = {}
	local ear_2_check_table = {}
	local ring_1_check_table = {}
	local ring_2_check_table = {}
	ear_1_check_table = fill_in_check_table(ear_1_check_table, gear_list.ear1, player_job)
	ear_2_check_table = fill_in_check_table(ear_2_check_table, gear_list.ear2, player_job)
	ring_1_check_table = fill_in_check_table(ring_1_check_table, gear_list.ring1, player_job)
	ring_2_check_table = fill_in_check_table(ring_2_check_table, gear_list.ring2, player_job)
	check_for_gear_in_multiple_slots(ear_1_check_table, gear_list.ear2, player_job, "ear1", "ear2")
	check_for_gear_in_multiple_slots(ear_2_check_table, gear_list.ear1, player_job, "ear2", "ear1")
	check_for_gear_in_multiple_slots(ring_1_check_table, gear_list.ring2, player_job, "ring1", "ring2")
	check_for_gear_in_multiple_slots(ring_2_check_table, gear_list.ring1, player_job, "ring2", "ring1")
	windower.add_to_chat(122, "Check finished")
end
...
function parse_command(...)
	...
	if args[1] == "mastergear" then
		...
		elseif args[1] == "checkmultislot" then
			if args[2] then 
				check_multi_slots(string.upper(args[2]))
			else
				check_multi_slots(player.main_job)
			end
		else
			print_help()
		end
